...
- [~] **M5.3** Build reference PCA from **1000 Genomes** on array-overlapping markers;
      ...
      - **Not yet run against the real panel.** 1000 Genomes is still downloading; the tests
        run against a PLINK stub, which holds the orchestration to account and is explicitly
        not a claim about what PLINK computes. The first real-panel run is the remaining
...
- [x] **M5.4** Project the sample onto reference PCs via `--score`
      ...
      - **A ragged `.sscore` is refused rather than padded**, found by writing a test whose
        ...
        a truncated file.
      - Eleven guards, each verified by neutering it. **Not yet run against the real panel**
        — the tests drive a PLINK stub, which holds the flag set to account and claims
        nothing about what PLINK computes from it.
