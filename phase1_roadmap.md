...
- [x] **M2.1** `manifest.yaml` schema (`refs/manifest.py`, `refs/licenses.py`,
      ...
      - **No URL templating.** 1000G names chromosome X `...v1c...` where the autosomes are
        ...
        is the feature.
      - Post-processing steps are **declared and validated, not executed** — there is no
        runner yet, so every step is `implemented=False` and the fetcher reports each one
        as outstanding. A typo still fails at manifest load rather than after a 60 GB
        download, which is the part that had to exist now.
...
- [x] **M3.2** Matcher (`engine/matcher.py`). Resolves card variant keys against the
      ...
      - **Duplicate probes are resolved by agreement, never by choice** — the decision M1
        ...
        does not contain. A no-call beside a call is not a disagreement.
...
| A declared post-processing step is never executed but reports as done | M2.1 | No executor exists yet, so every step is `implemented=False` and reported pending; wiring one is the job of the milestone that needs it | — |
