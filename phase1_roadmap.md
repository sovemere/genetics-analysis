...
- [ ] **M4.2** Save / load / list / delete runs. Bundles must be re-readable months later
      ...
      - **Save and load shipped with [M4.1](#m4--run-bundle--first-ui)**, along with
        `BundleVersionError` and its distinction from corruption. What remains: listing
        (skipping `.incoming-` staging directories by shape), deletion, and the CLI surface.
      - Note the standing constraint M4.1 introduced: `read_bundle` rejects unknown payload
        keys, so **adding a field to the payload requires bumping `BUNDLE_FORMAT_VERSION`
        in the same commit**. A test pins the payload's whole nested shape so the bump
        cannot be forgotten.
      - The compatibility rule the review settled: **payload keys may be added, never
        removed and never repurposed.** That is what lets a reader accept any bundle at or
        below its own version, so a bump does not orphan saved runs. A key whose meaning
        changes gets a new name.
      - Listing must skip `.incoming-` staging directories; run ids are already refused if
        they start with a dot, so the naming rule and the listing rule cannot disagree.
...
- [ ] **M4.10** **Offline guarantee, end-to-end half** — moved here from M2.7, which now
      ...
      - It belongs here and not earlier because this is the first milestone at which a
        "full run" exists to test. It belongs here and not at M15 because the guarantee
        should be enforced from the moment it is testable, not audited once at the end.
